...
{
    int index = 0;
    list_struct** hash_table = (list_struct**) calloc(HASH_TABLE_SIZE, sizeof(list_struct*));
    for (int i = 0; i < HASH_TABLE_SIZE; i++)
        hash_table[i] = make_list(START_LIST_SIZE);

    for (int i = 0; i < word_qty; i++)
    {
        index = cmptor(word_array[i]);
        append_to_list(hash_table[index], word_array[i]);
    }
    return hash_table;
}
...
{
    return new_strlen(word) % HASH_TABLE_SIZE;
}
...
int shift_right(char* word)
{
    ...
    for (int i = 0; i < new_strlen(word) - 1; i++)
    {
        result = ((result >> 1) | (result << 7));
        result = (result | (unsigned char)word[i]) & (255-(result & (unsigned char)word[i]));
    }
    return (int)result % HASH_TABLE_SIZE;
}
...
int crc32(char* word)
{
	...
	return (int)((crc ^ 0xffffffff ) % HASH_TABLE_SIZE);
